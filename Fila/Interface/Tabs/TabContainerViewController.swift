...
extension TabContainerViewController {
    static func runSelfCheck() {
        ...
        let compact = UIViewController(), wide = UIViewController()
        container.move(to: compact)
        let firstID = UUID(), secondID = UUID()
        let first = UIViewController(), editor = UIViewController(), second = UIViewController()
        let firstNavigation = UINavigationController(rootViewController: first)
        firstNavigation.pushViewController(editor, animated: false)
        container.showTab(firstID) { firstNavigation }
        container.showTab(secondID) { UINavigationController(rootViewController: second) }
        let secondNavigation = container.navigation
        container.showTab(firstID) {
            assertionFailure("A live tab was reconstructed")
            return UINavigationController()
        }
        assert(container.navigation === firstNavigation)
        assert(container.navigation?.topViewController === editor)
        assert(secondNavigation?.topViewController === second)
        assert(firstNavigation.parent === container && secondNavigation?.parent === container)
        let overview = UIViewController()
        container.display(overview)
        container.move(to: wide)
        assert(container.parent === wide && container.displayed === overview)
        container.showTab(firstID) {
            assertionFailure("Dismissing the overview reconstructed the tab")
            return UINavigationController()
        }
        assert(container.navigation?.topViewController === editor)
        assert(firstNavigation.parent === container && secondNavigation?.parent === container)
        assert(overview.parent == nil)
        assert(container.tabs.count == 2)
    }
}
#endif
