...
extension SearchViewController: UICollectionViewDelegate {
    ...
    extension SearchViewController {
        /// Exercises the real search UI state with cached entries, without filesystem I/O.
        static func runSelfCheck() {
            let note = FileNode(
                name: "notes.txt", kind: .regular, size: 1, allocatedSize: 1,
                modified: 0, created: 0, accessed: 0, mode: 0, ownerID: 0,
                groupID: 0, systemFlags: 0, linkCount: 1, inode: 1
            )
            let controller = SearchViewController(root: "/search-check", query: "NOTE", scope: .folder)
            controller.folderEntries = [note, note]
            controller.loadViewIfNeeded()
            let scopeItem = controller.navigationItem.rightBarButtonItem
            assert(scopeItem?.style == .plain && scopeItem?.image != nil && scopeItem?.customView == nil)
            assert(scopeItem?.menu?.children.count == 2)
            assert(controller.navigationItem.searchController?.hidesNavigationBarDuringPresentation == false)
            if #available(iOS 26.0, *) {
                assert(controller.toolbarItems?.first === controller.navigationItem.searchBarPlacementBarButtonItem)
            }
            assert(controller.hits.map(\.node.name) == ["notes.txt"], "A live listing can repeat a name across pages")
            guard let bar = controller.navigationItem.searchController?.searchBar else { assertionFailure(); return }
            assert(bar.scopeButtonTitles == nil && !bar.showsScopeBar, "Search scope belongs in the menu")
            controller.searchBar(bar, textDidChange: "absent")
            assert(controller.hits.isEmpty && !controller.isSearching)
            bar.text = "notes"
            controller.selectScope(.subfolders)
            assert(controller.hits.isEmpty && !controller.isSearching && controller.walk == nil, "Changing scope must not start a subtree walk")
            controller.selectScope(.folder)
            assert(controller.hits.map(\.node.name) == ["notes.txt"])
            controller.searchBar(bar, textDidChange: "")
            assert(controller.hits.isEmpty && controller.query.isEmpty)
            controller.searchBarCancelButtonClicked(bar)
            assert(controller.walk == nil && !controller.isSearching && !controller.spinner.isAnimating)
            assert(bar.searchTextField.leftView === controller.magnifier, "An idle search field shows its magnifier, not a spinner")
            assert(controller.view.subviews.contains(controller.collectionView), "Results are the whole page: no summary header above them")
            assert(!controller.hasFooterLayout, "A complete list carries no truncation footer")
            controller.scope = .subfolders
            controller.hits = (0 ..< FileSearch.resultLimit).map { FileSearchResult(directory: "/search-check/\($0)", node: note) }
            controller.apply()
            assert(controller.hasFooterLayout, "A truncated result list never looks like a complete one")
            controller.hits = [FileSearchResult(directory: "/search-check", node: note)]
            controller.skippedLinks = 1
            controller.apply()
            assert(controller.hasFooterLayout, "A walk that passed over a link says so")
            assert(controller.footerText?.contains("\n") == false, "Only the link note applies here")
            controller.start("")
            assert(controller.skippedLinks == 0 && !controller.hasFooterLayout)
        }
    }
#endif
