...
public enum SharedInbox {
    public static func directory(in group: URL) throws -> URL {
        ...
        let operations = FileOperations(bootstrapRoot: group, writableRoot: group)
        var status = stat()
    ...
    public static func save(_ source: URL, suggestedName: String?, in inbox: URL) throws {
        ...
        let operations = FileOperations(bootstrapRoot: inbox.path, writableRoot: inbox.path)
        for index in 0..<1000 {
            ...
            do {
                try operations.copyRegularFile(at: source.path, to: inbox.appendingPathComponent(candidate).path)
                return
