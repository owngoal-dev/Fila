...
public extension FileOperations {
    ...
    func copyRegularFile(at source: String, to destination: String) throws {
        ...
        let temporary = try resolveForWrite(FilaPath.join(FilaPath.directory(of: destination), ".fila-provider-" + UUID().uuidString))
        ...
        do {
            ...
            let output = Darwin.open(temporary, cloned ? O_RDONLY | O_NOFOLLOW : O_WRONLY | O_CREAT | O_EXCL | O_NOFOLLOW, 0o600)
            guard output >= 0 else { throw FilaFailure(errno: errno, path: destination) }
            ownsTemporary = true
            defer { close(output) }
            if !cloned {
                guard fcopyfile(input, output, nil, copyfile_flags_t(COPYFILE_ALL)) == 0 else { throw FilaFailure(errno: errno, path: destination) }
            }
            var copied = stat()
            ...
            if copied.st_flags & immovable != 0 {
                guard fchflags(output, copied.st_flags & ~immovable) == 0 else { throw FilaFailure(errno: errno, path: destination) }
            }
            while fsync(output) != 0 {
                if errno != EINTR { throw FilaFailure(errno: errno, path: destination) }
            }
            guard renamex_np(temporary, destination, UInt32(RENAME_EXCL)) == 0 else { throw FilaFailure(errno: errno, path: destination) }
            ownsTemporary = false
            if copied.st_flags & immovable != 0 {
            ...
            if ownsTemporary {
                ...
                guard unlink(temporary) == 0 || errno == ENOENT else { throw FilaFailure(errno: errno, path: temporary) }
            }
            throw error
        }
    }
