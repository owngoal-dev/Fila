    ...
    open class BackendListViewController<Item: Hashable & Sendable>: TabContentViewController, PreparableContent {
        ...
        open func makeCell(_: UICollectionView, at _: IndexPath, for _: Item) -> UICollectionViewCell {
        ...
        open func loadDidFail(_: Error, hadRows _: Bool) {}
        ...
        open func loadDidComplete(received _: Int, elapsed _: TimeInterval, failed _: Bool) async {}
            ...
            loadTask = Task { [weak self] in
                ...
                do {
                    for try await batch in load() {
                        ...
                        // of them.
                        if let lastApplyAt {
                            if DispatchTime.now() < lastApplyAt + BackendListPacing.applyInterval {
                                continue
                            }
