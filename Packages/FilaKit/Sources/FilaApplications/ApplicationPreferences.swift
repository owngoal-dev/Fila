...
/// What the applications backend remembers: the user's switch for the
/// feature and the list's own order and scope, separate from the file
...
public struct ApplicationPreferences: Codable, Equatable, Sendable {
    ...
    public init(showsApplications: Bool = true, sort: AppSort = .name, scope: AppScope = .all) {
        self.showsApplications = showsApplications
        self.sort = sort
        self.scope = scope
    }
}
...
/// The keys the app has always written for these three switches:
/// `showsApplications`, `appSort` and `appScope`, each in its old shape.
/// Only the keys whose value changed are written.
@MainActor
public final class ApplicationPreferencesDefaults: DefaultStorage {
    ...
    public func load() throws -> ApplicationPreferences? {
        let value = ApplicationPreferences(
            showsApplications: defaults.object(forKey: "showsApplications") as? Bool ?? true,
    ...
    public func save(_ value: ApplicationPreferences) throws {
        if known?.showsApplications != value.showsApplications {
            defaults.set(value.showsApplications, forKey: "showsApplications")
        }
