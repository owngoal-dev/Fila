...
extension Negotiate.Dialects: CustomDebugStringConvertible {
    public var debugDescription: String {
        switch self {
        case .smb202:
            "SMB 2.0.2 (\(String(format: "0x%04x", rawValue)))"
        case .smb210:
            "SMB 2.1.0 (\(String(format: "0x%04x", rawValue)))"
        case .smb300:
            "SMB 3.0.0 (\(String(format: "0x%04x", rawValue)))"
        case .smb302:
            "SMB 3.0.2 (\(String(format: "0x%04x", rawValue)))"
        case .smb311:
            "SMB 3.1.1 (\(String(format: "0x%04x", rawValue)))"
        }
    }
}
