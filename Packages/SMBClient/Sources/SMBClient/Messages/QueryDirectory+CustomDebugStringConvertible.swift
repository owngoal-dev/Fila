...
extension QueryDirectory.FileInformationClass: CustomDebugStringConvertible {
    public var debugDescription: String {
        ...
        case .fileDirectoryInformation:
            "SMB2_FILE_DIRECTORY_INFO (\(String(format: "0x%02x", rawValue)))"
        case .fileFullDirectoryInformation:
            "SMB2_FILE_FULL_DIRECTORY_INFO (\(String(format: "0x%02x", rawValue)))"
        case .fileIdFullDirectoryInformation:
            "SMB2_FILE_ID_FULL_DIRECTORY_INFO (\(String(format: "0x%02x", rawValue)))"
        case .fileBothDirectoryInformation:
            "SMB2_FILE_BOTH_DIRECTORY_INFO (\(String(format: "0x%02x", rawValue)))"
        case .fileIdBothDirectoryInformation:
            "SMB2_FILE_ID_BOTH_DIRECTORY_INFO (\(String(format: "0x%02x", rawValue)))"
        case .fileNamesInformation:
            "SMB2_FILE_NAME_INFO (\(String(format: "0x%02x", rawValue)))"
        case .fileIdExtdDirectoryInformation:
            "SMB2_FILE_ID_EXTD_DIRECTORY_INFO (\(String(format: "0x%02x", rawValue)))"
        case .fileInfomationClass_Reserved:
            "SMB2_FILE_INFORMATION_CLASS_RESERVED (\(String(format: "0x%02x", rawValue)))"
        }
    }
}
