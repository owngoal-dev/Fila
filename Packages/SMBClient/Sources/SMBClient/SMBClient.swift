...
public class SMBClient {
    ...
    public func logoff() async throws -> Logoff.Response {
        ...
        return try await session.logoff()
    }
    ...
    public func download(path: String) async throws -> Data {
        try await download(path: path, progressHandler: { _ in })
    }
    ...
    public func availableSpace() async throws -> UInt64 {
        ...
        return availableAllocationUnits * bytesPerAllocationUnit
    }
...
public struct File: Hashable {
    ...
    public var lastAccessTime: Date {
        fileStat.lastAccessTime
    }

    public var lastWriteTime: Date {
        fileStat.lastWriteTime
    }
