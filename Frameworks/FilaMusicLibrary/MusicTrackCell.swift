...
/// One song row: cover, title over artist (and album, where there is room),
/// duration at the trailing edge.
final class MusicTrackCell: UICollectionViewListCell {
    ...
    func show(_ track: MusicLibraryTrack) {
        self.track = track
        titleLabel.text = track.title.isEmpty ? String(localized: "Untitled", bundle: MusicLibraryBackend.bundle) : track.title
