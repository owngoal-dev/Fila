...
This is the user's explicit revision of the earlier “one app binary, four wrappers” rule for the ordinary IPA. Deb and tipa retain full composition and runtime daemon/local resolution. `make ipa` will select FilaSandboxed when implemented; `make packages` must build/cache both compositions correctly. Version remains in Version.xcconfig, projects remain hand-maintained, and the existing app-group/File Provider contract remains required. Scheme/product names here are planned, not created.
