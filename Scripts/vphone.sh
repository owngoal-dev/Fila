...
try:
    ...
    with TemporaryDirectory(prefix="fila-vphone-") as directory:
        ...
        with ThreadingHTTPServer((str(host), port), handler) as server:
            ...
            print(f"2. Open Files > Recents, tap {filename}, then install it in your preferred package manager.")
            print("3. After installation, close the old Fila in the App Switcher and reopen Fila.")
            print("Same-version rebuilds: remove only Fila's old .deb from the package manager's APT cache in Fila before installing.")
